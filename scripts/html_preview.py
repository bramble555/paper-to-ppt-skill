...
def generate_html_from_detail(detail_path: Path, output_path: Path, template_path: Path | None = None) -> None:
    ...
    deck_title = next((ln.replace("论文标题：", "").strip() for ln in detail_text.splitlines() if ln.startswith("论文标题：")), "论文汇报")
    html = template.render(deck_title=deck_title, slides=slides, section_label="汇报页")
